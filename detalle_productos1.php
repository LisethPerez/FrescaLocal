...
$consult = "SELECT *, empleado.nombre AS nombreEmple, detalle_factura.cantidad AS cantidadd, detalle_factura.total AS total, detalle_factura.fecha AS fecha FROM detalle_factura INNER JOIN empleado ON detalle_factura.empleado_id_empleado=empleado.id_empleado INNER JOIN stock ON detalle_factura.stock_id_stock=stock.id_stock WHERE detalle_factura.factura_id_factura={$id}";
...
if($num = $sql->num_rows>0){
    ...
    while($row = mysqli_fetch_assoc($sql)){
        ...
        $datos[] = array(
            ...
            "producto"=>$nombrePro,
            "precio_venta"=> "$".number_format($row['precio_venta']),
            "precio_total"=>"$".number_format($row['total']),
